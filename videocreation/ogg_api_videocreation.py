...
def create_overlay(size, overall_styles, title_data, description_data, credits_data, webstories_title_data=None, is_first_slide=False):
    ...
    scale_factor = size[1] / 800 # Assuming the original height was 1920
    ...
    default_font_size = int(20 * scale_factor)

    # Adjust these values to change the font sizes relative to default_font_size
    ...
    line_spacing = int(5 * scale_factor)  # Adjust this value to increase/decrease space between title and description
    credits_spacing = int(1.2 * scale_factor)  # New variable for spacing before credits (reduced from 5)
    ...
    top_left_radius, top_right_radius, _, _ = map(int, re.findall(r'\d+', border_radius))
    top_left_radius = int(top_left_radius * scale_factor)
    top_right_radius = int(top_right_radius * scale_factor)

    # Create rounded rectangle overlay at the bottom of the image
    overlay_top = overlay_size[1] - overlay_height
    rounded_rectangle(draw, [(0, overlay_top), overlay_size], top_left_radius, fill=color)
...
def process_amp_story_pages(url):
    ...
    try:
        ...
        if not pages:
            logger.error(f"No amp-story-pages found in the URL: {url}")
            return None, None

        unique_id = content_id[-6:]
        story_data = []
        ...
        for index, page in enumerate(pages, start=1):
            ...
            if not img_element:
                logger.warning(f"No image element found for slide {index}")
                continue
            ...
            if text_overlay:  # Only process text if overlay exists
                if index == 1:  # Special handling for the first slide
                    secname_element = text_overlay.find('p', class_='secname')
                    title_element = text_overlay.find('h1') or text_overlay.find('h2')
            ...
            page_data = {
                ...
                "overall_styles": extract_styles(text_overlay) if text_overlay else {},
                "title_styles": extract_styles(title_element) if title_element else {},
                "description_styles": extract_styles(description_element) if description_element else {},
                "credit_styles": extract_styles(credit_element) if credit_element else {},
...
def main(url, bgm_path, zoom_factor, zoom_duration, bgm_volume, silence_duration, use_tts=True, output_format='vertical', intro_path=None, end_credits_path=None):    
    ...
    try:
        ...
        print("Step 7: Copying new app files to the 'contentgrowth' bucket...")
        if main_folder:
            try:
...
def create_video_api():
    ...
    zoom_factor = request.json.get('zoom_factor', 1.2)
    zoom_duration = request.json.get('zoom_duration', 30)

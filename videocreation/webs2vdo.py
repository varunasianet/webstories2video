...
def create_video(image_folder, output_video_path, target_duration, overlay_clips=None, bgm_path=None, bgm_volume=None, audio_folder=None, silence_duration=3, output_format='default', intro_path=None, end_credits_path=None):
    ...
    # Set audio codec to AAC and video codec to H.264
...
def main(url, bgm_path, zoom_factor, zoom_duration, bgm_volume, silence_duration, use_tts=True, output_format='vertical', intro_path=None, end_credits_path=None):
    try:
        ...
        content_id = unique_id  # Define content_id here

        # Update the content_id in the story_data
        for slide in story_data:
            ...
            if os.path.exists(image_path):
                ...
                if use_tts:
                    ...
                    if success:
                        ...
                        total_tts_duration += audio_duration
        ...
        # Calculate target duration based on TTS durations and silence
        target_duration = total_tts_duration + (silence_duration * num_slides)
        
        # Process images with the calculated durations
        for i, slide in enumerate(story_data):
            image_path = os.path.join(image_folder, slide['image']['local_path'])
            if os.path.exists(image_path):
                img = Image.open(image_path)
                ...
                slide_duration = tts_durations[i] + silence_duration
        ...
        total_audio_duration = sum(tts_durations) + (silence_duration * num_slides)
        ...
        print(f"Total video duration: {total_audio_duration + intro_duration + end_credits_duration:.2f} seconds")
...
if __name__ == "__main__":
    url = "https://newsable.asianetnews.com/webstories/lifestyle/do-you-drink-cinnamon-tea-7-benefits-you-need-to-know-dmn-siipyr"
    ...
    silence_duration = 1
    use_tts = True
    intro_path = "/home/varun_saagar/videocreation/artificts/intro-fixed.mp4"
    end_credits_path = "/home/varun_saagar/videocreation/artificts/Newsable end sting__Vertical.mp4"
